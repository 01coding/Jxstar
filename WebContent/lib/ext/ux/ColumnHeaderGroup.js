...
Ext.ux.grid.ColumnHeaderGroup = Ext.extend(Ext.util.Observable, {
    ...
    viewConfig: {
        initTemplates: function(){
            ...
            if(!ts.gcell){
                ts.gcell = new Ext.XTemplate('<td class="x-grid3-hd x-grid3-gcell x-grid3-td-{id} ux-grid-hd-group-row-{row} {cls}" style="{style}">', '<div {tooltip} class="x-grid3-hd-inner x-grid3-hd-{id}" unselectable="on" style="{istyle}">', this.grid.enableHdMenu ? '<a class="x-grid3-hd-btn"></a>' : '', '{value}</div></td>');
            }
            this.templates = ts;
